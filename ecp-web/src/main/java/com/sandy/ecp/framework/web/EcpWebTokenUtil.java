...
/**
 * 企业云平台 JWT 会话保持签名加密工具
 * @author Sandy
 * @since 1.0.0 
 * @date 2024-12-30 14:12:22
 */
public final class EcpWebTokenUtil {
	...
	private static final String JWT_ISSUER = "ecp_issuer";
	...
	public static void setJwtSecert(final String jwtSecert) {
		EcpWebTokenUtil.jwtSecert = jwtSecert;
	}

	public static void setSignatureAlgorithm(final SignatureAlgorithm signatureAlgorithm) {
		EcpWebTokenUtil.signatureAlgorithm = signatureAlgorithm;
	}

	public static SecretKey generalKey() {
		final byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(EcpWebTokenUtil.jwtSecert);
		final SecretKey signingKey = new SecretKeySpec(apiKeySecretBytes, EcpWebTokenUtil.signatureAlgorithm.getJcaName());
		return signingKey;
	}

	public static String createJWTtoken(final String id, final String audience, final String subject,
			final Map<String, Object> claims, final Long ttlMillis) {
		final SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;
		final long nowMillis = System.currentTimeMillis();
		final Date now = new Date(nowMillis);
		final SecretKey secretKey = generalKey();
		final JwtBuilder builder = Jwts.builder().setId(id).setAudience(audience).setClaims((Map<String, Object>) claims)
				.setSubject(subject).setIssuer(JWT_ISSUER).setIssuedAt(now)
				.signWith(signatureAlgorithm, (Key) secretKey);
	...
	public static String createJWTtoken(final String id, final String audience, final String subject,
			final Map<String, Object> claims, final Date expDate) {
		final SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;
		final long nowMillis = System.currentTimeMillis();
		final Date now = new Date(nowMillis);
		final SecretKey secretKey = generalKey();
