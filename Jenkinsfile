...
pipeline {
    ...
    stages {
        ...
        stage('Duplicate deploy check') {
            steps {
                script {
                    def deployedCommitId = sh(returnStdout: true, script: "curl --silent https://builds.apache.org/job/Apache%20ecp/job/${env.JOB_BASE_NAME}/lastSuccessfulBuild/artifact/DEPLOY_COMMIT_ID || true").trim()
        ...
        stage('Commit status check') {
            ...
            steps {
                script {
                    ...
                    def commitStatusJson = sh(script: "curl --silent https://api.github.com/repos/apache/ecp/commits/$commitId/status", returnStdout: true).trim()
        ...
        stage('Snapshot version check') {
            ...
            steps {
                sh 'env'
                sh 'java -version'
                sh './mvnw clean install -pl "ecp-dependencies-bom" && ./mvnw clean install -DskipTests=true && ./mvnw clean validate -Psnapshot-ci-deploy -pl "ecp-all"'
            }
        }
        ...
        stage('Deploy snapshot') {
            ...
            steps {
                timeout(40) {
                    sh './mvnw --version'
                    sh './mvnw clean package deploy -pl ecp-dependencies-bom && ./mvnw clean source:jar javadoc:jar package deploy -DskipTests=true'
                }
            }
        }
    ...
    post {
        failure {
            ...
            URL: ${env.BUILD_URL}''', cc: '', from: '', replyTo: '', subject: 'Apache ecp snapshot deployment fail', to: 'dev@ecp.apache.org'
        }
    }
